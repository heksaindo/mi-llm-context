...
$(function() {
	...
	$('#print_engine_report').click( function() {
		...
		window.open("<?php echo base_url(); ?>engine_report/running_cetak/"+addParam, "", "scrollbars=1,height=700, width=1000"); 
			    ...
			    <div class="crumbs">
		            <ul id="breadcrumbs" class="breadcrumb"> 
		                <li><a href="<?php echo base_url(); ?>Home">Dashboard</a></li>
						 ...
						 <li class="active"><a href="<?php echo base_url(); ?>engine_report/running/<?php echo $info_report['id_report']; ?>" title=""><?php echo $info_report['report_name']; ?></a></li>
		            </ul>
									...
									?>
										...
										foreach($header_data_ready as $dtHeader){
											...
											if(!empty($dtHeader['ref_field'])){
												...
												if($dtHeader['output_format'] == 'date'){
													$dt_value = date("d-m-Y", strtotime($dt_value));
													echo '<td>'.$dt_value.'</td>';
												}else
												if($dtHeader['output_format'] == 'currency'){
													$dt_value = number_format_id($dt_value);
													echo '<td>'.$dt_value.'</td>';
												}else
												if($dtHeader['output_format'] == 'total'){
