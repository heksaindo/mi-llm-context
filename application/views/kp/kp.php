...
$(document).ready(function(){
	...
	if(!empty($data_pendidikan)){
		foreach($data_pendidikan as $dt){
			$dropdown_pendidikan .= '<option value="'.$dt->nama_strata.'">'.$dt->nama_strata.'</option>';
		}
	}
	...
	$( "#table-src_bulan, #table-src_tahun, #table-src_pendidikan" ).change(function() {
		...
		if(id_strata){
			param.push(id_strata);
		}
		var str = param.join(" , ");
		oTable_kp.fnFilter(str,null,false,true,false);
		//if(bulan){
...
function change_status(newstatus, pegawai_id, id, nip_baru, nama, before_status, status_masalah){
	...
	if(status_masalah == '' || status_masalah == 'N'){
		//alert(before_status);
		if(newstatus == 'Info Di Biro'){
			window.open('http://ropeg.kemkes.go.id/inpro/cek/kp_reguler.html', '_blank');
			/*
			$( "#masalahnya" ).val('');
		...
		if(newstatus == 'SK Selesai'){ 
			...
			if(before_status == ''){
				if(newstatus == 'Proses Entry Data' || newstatus == 'Tanda Tangan Usulan' || newstatus == 'Kirim Ke Biro' || newstatus == 'SK Selesai' ){
					do_change = 0;
					alert('Silahkan pilih Dokumen Usulan Masuk terlebih dahulu!');
				}
				if(newstatus == 'Dokumen Usulan Masuk'){
					if(confirm('Apakah '+nama+' akan diubah status menjadi "'+newstatus+'" ?')){
						$.ajax({
							type: "POST",
							url: "<?php echo base_url(); ?>kenaikanpangkat/do_insert",
			...
			if(before_status == 'Dokumen Usulan Masuk'){
				if(newstatus == 'Tanda Tangan Usulan' || newstatus == 'Kirim Ke Biro' || newstatus == 'SK Selesai' ){
					...
					if(newstatus == 'Proses Entry Data'){
						window.location.assign("<?php echo base_url().'kenaikanpangkat/addexisting/'; ?>"+pegawai_id+"/"+id);
					}
				}
			}
