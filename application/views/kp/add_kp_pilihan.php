...
$(document).ready(function(){
		...
		//$("#tanggal_sk").val('');	
		//$("#tanggal_berlaku").val('');	
		//$("#pejabatpenetapan").val('');	
		
		//autocomplete Pegawai
		$("#nama").autocomplete("<?php echo base_url(); ?>kenaikangajipegawai/auto_pegawai/", {
			width: 250,
			minChars:1,
			max:100,
			selectFirst: false
		...
		$("#nama").result(function(event, data, formatted) {
			if (data){  
				...
				$("#kantor").val(data[4]);	
				//$("#golongan").val(data[5]);	
				//$("#masa_kerja_gol").val(data[6]);	
				//$("#gapok_lama").val(data[7]);	
				//$("#no_sk").val(data[8]);
				//$("#tanggal_sk").val(data[9]);	
				//$("#pejabatpenetapan").val(data[10]);	
				//$("#tmt_kgb").val(data[11]);	
				//$("#tmt_kp").val(data[12]);	
		...
		$( "#simpan-kp" ).click(function(){
			...
			if(err == 0){
				$.ajax({
					type: "POST",
					url: "<?php echo base_url(); ?>kenaikanpangkat/insert_pilihan",
